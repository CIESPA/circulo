...
for(const a of authors){
 ...
 const person={'@type':'Person','@id':`${routeUrl(site,p.route)}#person`,name:a.name,url:routeUrl(site,p.route),description:a.summary,image:a.socialImage?abs(site,a.socialImage):undefined,email:a.contactEmail?`mailto:${a.contactEmail}`:undefined,sameAs:a.websiteUrl?[a.websiteUrl]:undefined,memberOf:{'@id':`${site.domain}/#organization`},knowsAbout:a.genre};
 ...
 const body=`<nav class="breadcrumbs container" aria-label="Migas de pan"><a href="/">Inicio</a><span>/</span><a href="/integrantes/">Integrantes</a><span>/</span><span>${escapeHtml(a.name)}</span></nav><section class="profile-hero"><div class="container profile-grid">${image}<div><p class="page-hero-label">${escapeHtml(a.group)}</p><h1>${escapeHtml(a.name)}</h1><p class="profile-summary">${escapeHtml(a.summary)}</p><p class="card-kicker">${escapeHtml(a.genre)}</p>${(a.contactEmail||a.websiteUrl||a.contactUrl)?`<div class="profile-actions">${a.contactEmail?`<a class="btn-secondary" href="mailto:${escapeHtml(a.contactEmail)}">Correo</a>`:''}${a.websiteUrl?`<a class="btn-secondary" href="${escapeHtml(a.websiteUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(a.websiteLabel||'Sitio web')}</a>`:''}${a.contactUrl?`<a class="btn-secondary" href="${escapeHtml(a.contactUrl)}">${escapeHtml(a.contactLabel||'Contacto')}</a>`:''}</div>`:''}</div></div></section><section class="content-section"><div class="container profile-content"><article class="prose"><p class="section-label">Biografía</p>${md(a.biography)}</article>${textsHtml}${booksHtml}</div></section>`;
